...
public class CmdUtil {
	...
	/**
	 * return null if selectFile don't exists on disk ��if selectedFile is a
	 * file then return it's parent�� if selectFile is a dir then return itself
	...
	public static String[] parseCmdPattern(String cmdPattern, File selectedFile) {
		...
		for (int i = 0; i < cmdAndParams.length; i++) {
			if (cmdAndParams[i].contains("%f")) {
				cmdAndParams[i] = cmdAndParams[i].replace("%f",
						selectedFile.getAbsolutePath());
			}
			if (cmdAndParams[i].contains("%d")) {
				cmdAndParams[i] = cmdAndParams[i].replace("%d",
						getDir(selectedFile).getAbsolutePath());
			}
		}
